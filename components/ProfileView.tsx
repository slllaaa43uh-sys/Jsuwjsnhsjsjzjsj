...
const ProfileView: React.FC<ProfileViewProps> = ({ onClose, onReport, userId, onLogout }) => {
  ...
  const fetchContent = useCallback(async (type: 'all' | 'video', signal: AbortSignal, pageNum: number) => {
    ...
    if (isLoadMore) {
        ...
        // FIXED: Only set loading if we don't have data already to avoid flash
        if (type === 'all' && posts.length === 0) setLoadingContent(true);
        if (type === 'video' && videos.length === 0) setLoadingContent(true);
    }
  ...
  // Content Fetching Logic - UPDATED TO PREVENT DUPLICATES ON TAB SWITCH
  useEffect(() => {
    ...
    // Loading State Logic
    if (activeTab === 'posts' && posts.length === 0) setLoadingContent(true);
    else if (activeTab === 'videos' && videos.length === 0) setLoadingContent(true);
    else setLoadingContent(false);

    // Fetching Logic with Ref Guard
    if (activeTab === 'posts') {
        if (posts.length === 0) {
  ...
  }, [activeTab, postsPage, videosPage]); // Dependencies updated
  ...
  const handleTabChange = (tab: 'posts' | 'videos') => {
    ...
    // Prevent loading flash by checking existing data immediately
