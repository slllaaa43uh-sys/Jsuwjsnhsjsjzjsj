...
import { 
  ...
  Play, Loader2, Flag, ArrowRight, ChevronDown, Trash2, Copy, MoreHorizontal, Sparkles, Edit2, Globe, Lock, Repeat, Check, ToggleLeft, ToggleRight, Save, CornerDownLeft
} from 'lucide-react';
...
const ShortsView: React.FC<ShortsViewProps> = ({ 
        ...
        let mappedShorts: ShortVideo[] = postsArray
            ...
            .map((item: any): ShortVideo => {
                ...
                return {
                    ...
                    description: combinedDesc, // Using combined description
  ...
  const handleFeedTouchMove = (e: React.TouchEvent) => {
      ...
      // Logic for drag effect if needed
  };
  ...
  const handleUndoRepost = async () => {
      ...
      // Optimistic update: set isReposted to false, decrement count (-1)
                 ...
                 if (isReply && parentId) {
                     setComments(prev => prev.map(c => {
                         if (c._id === parentId) {
                             return {
                                 ...c,
                                 replies: c.replies?.map(r => r._id === tempId ? { ...r, _id: realCommentData._id || realCommentData.id, pending: false } : r)
                             };
                         }
                         return c;
                     }));
                 } else {
                     setComments(prev => prev.map(c => c._id === tempId ? { ...c, _id: realCommentData._id || realCommentData.id, pending: false } : c));
                 }
             } else {
                 if (isReply && parentId) {
                     setComments(prev => prev.map(c => c._id === parentId ? {...c, replies: c.replies?.map(r => r._id === tempId ? {...r, pending: false} : r)} : c));
                 } else {
  ...
  const handleReplyClick = (e: React.MouseEvent, comment: Comment) => {
      e.stopPropagation();
  ...
  const renderCommentItem = (comment: Comment, isReply = false, parent: Comment | null = null) => {
    ...
    return (
      <div 
        key={comment._id} 
        className={`flex flex-col mb-4 ${isReply ? 'mt-2 mr-8 pl-0 border-r-2 border-gray-100 pr-2' : ''} ${comment.pending ? 'opacity-70' : 'opacity-100'}`}
      >
        <div 
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-4 mt-1 px-1">
                    ...
                    {!comment.pending && (
                        <button 
                            onClick={(e) => {
                                handleReplyClick(e, isReply && parent ? parent : comment);
                            }}
            ...
            <div className="bg-white w-full max-w-md h-[70vh] rounded-t-3xl relative z-10 animate-in slide-in-from-bottom duration-300 flex flex-col p-6 pb-safe">
                ...
                <div className="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
                    <h3 className="text-lg font-bold text-gray-900">
                        {language === 'ar' ? 'تعديل إعدادات الفيديو' : 'Edit Video Settings'}
                    </h3>
                        ...
                        <div className="space-y-3">
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="text-start">
                                        <h4 className="font-bold text-sm">{language === 'ar' ? 'للجميع' : 'For everyone'}</h4>
                                    </div>
                                </div>
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="text-start">
                                        <h4 className="font-bold text-sm">{language === 'ar' ? 'لك (خاص)' : 'For you (Private)'}</h4>
                                    </div>
                                </div>
                ...
                <div className="pt-4 border-t border-gray-100">
                    ...
                    >
                        ...
                        {isSavingSettings ? t('saving') || 'Saving...' : t('save')}
                    </button>
                </div>
               ...
               <div className="p-3 border-t border-gray-100 bg-white pb-safe z-40 relative">
                  {replyingTo && (
                        <div className="flex items-center justify-between px-2 mb-2 text-xs text-gray-500 bg-gray-50 p-2 rounded-lg border border-gray-100">
                            ...
                            <button onClick={() => { setReplyingTo(null); setCommentText(''); }} className="p-1 hover:bg-gray-200 rounded-full"><X size={12} /></button>
                        </div>
                  )}
