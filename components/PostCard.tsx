...
const PostCard: React.FC<PostCardProps> = ({ post, variant = 'feed', onDelete, onReport, onProfileClick, isActive = true }) => {
  ...
  // --- RENDER ---
  const displayLocation = (post.location === 'عام' || post.location === 'General') ? t('location_general') : (post.location || t('location_general'));
        ...
        <div className="px-4 flex justify-between items-start mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex flex-col gap-0.5"><div className="flex items-center gap-1"><span className="text-xs text-gray-500">{post.timeAgo}</span><span className="text-gray-300 text-[10px]">•</span><span className="text-xs text-gray-400">{displayLocation}</span></div></div>
            </div>
          </div>
